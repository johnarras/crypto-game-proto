...
public class GameObjectUtils
{
    ...
    public static List<T> GetComponents<T>(GameObject go) where T : Component
    {
        ...
        arr = go.GetComponentsInChildren<T>();
        if (arr != null && arr.Length > 0)
        {
            foreach (T comp in arr)
            {
    ...
    public static C GetOrAddComponent<C>(GameObject go) where C : Component
    {
        if (go == null)
        {
            return null;
    ...
    public static GameObject InstantiateIntoParent(object child, GameObject parent)
    {
        ...
        // Uncomment this if you are missing components.
        //Debug.LogError("InstParent: " + go.name);
